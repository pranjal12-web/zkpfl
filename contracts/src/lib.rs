...
fn add_weights(
    ...
    assert!(
        ...
        "Each address is only allowed to send one Weights variable. Sender: {:?}",
        context.sender
    );
    let input_def = ZkInputDef::with_metadata(SecretVarType::Weights {});
    (state, vec![], input_def)
}
...
/// Allows the administrator to start the computation of the average Weights.
///
